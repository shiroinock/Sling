...
- **ビジュアルキーボードUI**:
  ...
  - ISO/JIS Enterキーの特殊形状対応
  - キーコンポーネントの構造改善（外枠とパディングの分離）
  - レイアウト切り替え時の自動再マウント
...
- **ビジュアルキーボードレイアウトの永続化**:
  - 選択したキーボードレイアウトをローカルストレージに保存
  - ページ読み込み時に前回選択したレイアウトを自動復元
- **バックアップ/リストア機能**:
  - Zustandストアを使用した設定のバックアップ管理
  - 最大10個のバックアップを自動管理
  - バックアップの名前変更・説明追加
  - 個別バックアップのJSONエクスポート
- **UI言語の英語化**:
...
### 緊急修正項目（GitHubイシュー）
- [x] **#6** ダークモード有効無効が効かない - ダークモード切り替え機能の修正 - 完了
- [x] **#8** ビジュアルキーボードのキーの配置がズレている - キーレイアウトの位置調整 - 完了

### フェーズ0: UX改善と国際化（1週間）
- [x] **#4** karabiner.jsonを開いたとき、デフォルトのプロファイルのタブにフォーカスする - 完了
- [x] **#5** ビジュアルキーボードの配列の選択を記憶し、ページを開き直したときにも反映された状態にする - 完了
- [x] **#9** ビジュアルキーボードの配列、macbookのUS配列をデフォルトにする - 完了
...
- [ ] **デバイスプロファイル機能** （最優先）
  - デバイスごとの設定管理
  - vendor_id/product_idによるデバイス識別
  - デバイス別Simple Modifications
  - デバイス別Function Keys設定
  - デバイスの有効/無効切り替え
  - 内蔵キーボードと外部キーボードの区別
- [ ] キーボードショートカット対応
- [x] 設定のバックアップ/リストア機能 - 完了
  - 最大10個のバックアップを保持
  - バックアップの名前変更・説明追加機能
  - 個別バックアップのJSONエクスポート
  - Zustandストアでローカルストレージに永続化

### フェーズ1.5: 追加機能（低優先度）
- [ ] ルールのインポート/エクスポート（個別）
  - Complex Modificationsの個別ルール共有
  - JSONファイルとしてエクスポート
  - 既存設定を保持したままインポート
- [ ] キーマッピングのプリセット集
  - 開発者向けプリセット（Emacs/Vim風）
  - Mac最適化プリセット
  - ゲーミングプリセット

### フェーズ2: QMKレイヤー基本実装（2-3週間）【一部完了】
- [x] レイヤー管理システムの設計 - 完了
  - UUID ベースのレイヤー管理システム
  - ベースレイヤー（Layer 0）と追加レイヤーの概念
  - Zustand ストアによる状態管理
- [x] レイヤー設定の型定義 - 完了
  - `LayerConfiguration`、`Layer`、`LayerMapping` 型定義
  - `LayerAction` による mod-tap、layer-tap 等のアクション定義
  - Karabiner Complex Modifications への変換ロジック実装
- [x] 基本的なレイヤー切り替え機能 - 完了
  - MO(layer) - Layer Momentary（押している間だけレイヤー有効）
  - TG(layer) - Layer Toggle（レイヤーのトグル）
  - Layer-Tap - タップでキー、ホールドでレイヤー切り替え
  - Mod-Tap - タップでキー、ホールドでモディファイア
- [x] レイヤーUI実装 - 完了
  - レイヤータブによる管理 UI
  - レイヤーの追加・削除・名前変更
  - ビジュアルキーボードによるマッピング表示（Simple Modifications と統一）
  - レイヤーごとのマッピング編集機能
...
### フェーズ5: 高度なQMK機能（3-4週間）【一部完了】
- [x] Mod-Tap（MT）- 完了
  - タップで通常キー、ホールドでモディファイア
  - LayerActionSelector による設定 UI
- [x] Layer-Tap（LT）- 完了
...
- **本文**: `text-gray-900 dark:text-gray-200` (ダークモードのコントラスト改善)
...
- **キーボードキー背景**: `bg-white dark:bg-gray-700` (ダークモードの視認性向上)
- **ホバー背景**: `hover:bg-gray-50 dark:hover:bg-gray-600`
