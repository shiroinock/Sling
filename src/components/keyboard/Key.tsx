...
export function Key({
  ...
  // 内部のキービジュアルのサイズ（パディングを引いたサイズ）
  // ISO Enterキーの場合は特殊な調整
  const visualWidth = shape === 'iso-enter' ? containerWidth : containerWidth - padding * 2
  const visualHeight = shape === 'iso-enter' ? containerHeight : containerHeight - padding * 2
  ...
  return (
    ...
    >
      ...
      <button
        ...
        style={{
          top: shape === 'iso-enter' ? '0px' : `${padding}px`,
          left: shape === 'iso-enter' ? '0px' : `${padding}px`,
          ...
          >
            ...
            <path
              d={`
                M 7 3
                L ${visualWidth - 6} 3
                Q ${visualWidth - 2} 3 ${visualWidth - 2} 7
                L ${visualWidth - 2} ${visualHeight - 7}
                Q ${visualWidth - 2} ${visualHeight - 3} ${visualWidth - 6} ${visualHeight - 3}
                L ${((upperWidth - lowerWidth) / upperWidth) * visualWidth + 6} ${visualHeight - 3}
                Q ${((upperWidth - lowerWidth) / upperWidth) * visualWidth + 2} ${visualHeight - 3} ${((upperWidth - lowerWidth) / upperWidth) * visualWidth + 2} ${visualHeight - 7}
                L ${((upperWidth - lowerWidth) / upperWidth) * visualWidth + 2} ${visualHeight / 2 + 1}
                Q ${((upperWidth - lowerWidth) / upperWidth) * visualWidth + 2} ${visualHeight / 2 - 2} ${((upperWidth - lowerWidth) / upperWidth) * visualWidth - 2} ${visualHeight / 2 - 2}
                L 7 ${visualHeight / 2 - 2}
                Q 3 ${visualHeight / 2 - 2} 3 ${visualHeight / 2 - 6}
                L 3 7
                Q 3 3 7 3
                Z
              `}
              ...
              stroke={
                isSelected
                  ...
                  : isMapped
                    ? isDarkMode
                      ? 'rgb(34, 197, 94)'
                      : 'rgb(5, 150, 105)' // green-500 / green-600
                    : isDarkMode
                      ? 'rgb(75, 85, 99)'
                      : 'rgb(209, 213, 219)' // gray-600 / gray-300
              }
            />
          </svg>
        )}
